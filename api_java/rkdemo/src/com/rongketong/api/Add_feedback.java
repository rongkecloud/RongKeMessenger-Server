...
public class Add_feedback extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//参数格式校验
    	if(!Tools.checkSession(appSession) || !Arrays.asList(new String[]{"1", "2","3"}).contains(feedBackType) || (content.length() == 0 || content.length() > 140 || !Tools.containSpecChars(content))){
    		m_logger.info(String.format("FAILED params=%s %s",postParams,"check parameters error"));
