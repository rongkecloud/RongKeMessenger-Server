...
/*				if(ret == 1007){
	...
	public static Map<String,String> checkSession(String session) throws SQLException{
		...
		String sql = "SELECT b.session,a.user_account, a.user_pwd "
				+ "FROM user_accounts a,user_child_accounts b WHERE a.user_account=b.user_account and b.session = ?";
...
//		}
	 ...
	 * 系统登录
	 */
	public static HashMap<String,String> appLogin(String user_account,String user_pwd,String os){
...
//		//校验用户名是否已存在
		...
		String update_sql = "INSERT INTO user_child_accounts (user_account,os_type,session,created) "
				+ "VALUES(?,?,?,NOW()) ON DUPLICATE KEY UPDATE session=?";
		int eff_delete_count;
		try {
			eff_delete_count = MysqlBaseUtil.executeUpdateSql(update_sql,new String[]{user_account,os,appSession,appSession});
			if(eff_delete_count < 0){
				userMap.put("check_result", ApiErrorCode.SYSTEM_ERR); //系统内部错误
				return userMap;
			}
