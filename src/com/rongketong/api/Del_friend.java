 ...
 *				9999:参数错误
**/
@WebServlet("/del_friend.php")
public class Del_friend extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private static final Logger m_logger = Logger.getLogger(Del_friend.class);     
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    	 ...
    	 */
    	try {
			if(MysqlBaseManager.delFriend(accountInfo.get("user_account"), accountName)){
				//发送通知(此处需要调用云视互动服务推送消息)
				YsServerSDK client = Tools.geYsServerAPI();
				Integer resultCode = client.SendMessage(accountInfo.get("user_account"), accountName, "delete_friend");
				if(resultCode == 0){
					response.getWriter().write(ApiErrorCode.echoOk());
					return;
				}
			...
			m_logger.info(String.format("delete friend exception params=%s","error cause:"+e.getCause()));
		}
    	response.getWriter().write(ApiErrorCode.echoErr(ApiErrorCode.SYSTEM_ERR));
	}
